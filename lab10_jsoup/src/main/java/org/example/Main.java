    ...
    public static void main(String[] args) throws IOException {
        try{
            ...
            zakladyNazwy.forEach(element -> {
                if(element.text().startsWith("Zakład")){
                    ...
                    element.nextElementSibling().children().forEach(element1 -> {
                        if (!element1.text().equals("")) pr.add(element1.text().replace(",","").replace("dr ","").replace("inż. ","").replace("hab. ","").replace("mgr ","").replace(" prof. uczelni",""));
                        if (element1.text().startsWith("dr inż.") || element1.text().startsWith("dr") && !element1.text().equals("")) pr2.add(element1.text());
                    });
                    Collections.sort(pr);
            ...
            naglowkiRekt.forEach(element -> {
                godziny.add(new Godziny(element.select("a").attr("href"), new Date(element.select("small").text())));
            });
